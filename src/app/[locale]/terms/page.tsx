...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const { title } = (messages as any).TermsOfUse;

    return {
        title: `${title} | РесурсЛогистика`,
        description: locale === 'en'
            ? "Terms of Use for ResursLogistics digital services."
            : "Условия использования цифровых сервисов компании РесурсЛогистика.",
        alternates: {
...
export default async function TermsPage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
    const { messages } = await getServerTranslations(locale);
    const { title, content } = (messages as any).TermsOfUse;
